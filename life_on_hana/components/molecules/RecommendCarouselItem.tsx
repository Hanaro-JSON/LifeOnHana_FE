...
export function RecommendCarouselItem({ items }: { items: TRecommendCarouselItemProps[] }) {
      ...
      case "LOAN":
        return (
          <div className="flex flex-col items-end">
            ...
            <div className="font-SCDream8 text-sm text-[#4D00B5]">{item.maxAmount}</div>
          </div>
        );
      case "SAVINGS":
        return (
          <div className="flex flex-col items-end">
            ...
            <div className="font-SCDream8 text-[1rem] text-[#4D00B5]">최고 ~{item.maxInterest_rate}%</div>
          </div>
        );
      default:
        return null;
  ...
  const carouselItems = items.map((item, index) => (
    <div key={index} className="w-full h-full items-center flex flex-row justify-between">
