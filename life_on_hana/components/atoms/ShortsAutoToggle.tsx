...
interface TShortsAutoToggleProps {
    initialState: "play" | "pause";
    onToggle?: (state: "play" | "pause") => void;
}

export default function ShortsAutoToggle({
    initialState = "play",
    onToggle,
}: TShortsAutoToggleProps) {
    ...
    <div className="w-[1.8125rem] h-[.9375rem] absolute cursor-pointer select-none">
        ...
        >
        <Image
            className={`transition-opacity duration-300 ${selected === "play" ? "opacity-100" : "opacity-0"} bg-hanapurple`}
            src={play}
            alt="Play"
        />
        </div>
        ...
        >
        <Image
            className={`transition-opacity duration-300 ${selected === "pause" ? "opacity-100" : "opacity-0"} bg-hanapurple`}
            src={pause}
            alt="Pause"
        />
        </div>
